...
import { getScoreLabel, getImprovementTips } from "@/lib/scoring";
...
const ScanResults = () => {
  ...
  const radarData = Object.entries(scan.scores)
    ...
    .map(([key, val]) => ({ category: labels[key] || key, score: val }));
  ...
  return (
    <div className="min-h-screen bg-gradient-soft pb-8">
      ...
      <div className="px-5 space-y-5">
        {/* Overall Score Hero */}
        <motion.div
          className="bg-card rounded-3xl p-6 shadow-elevated text-center"
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.4 }}
        >
          <div className="flex justify-center mb-3">
            <ScoreRing score={scan.overall} size={120} strokeWidth={8} gradient />
          </div>
        ...
        >
          ...
          <div className="grid grid-cols-4 gap-y-5 gap-x-2">
            {Object.entries(scan.scores)
              .filter(([k]) => k !== "estimatedAge")
              .map(([key, val]) => (
                <ScoreRing key={key} score={val} size={56} strokeWidth={4} label={labels[key]} />
              ))}
          </div>
        </motion.div>
        ...
        <motion.button
          onClick={() => navigate("/")}
          className="w-full py-4 rounded-2xl bg-gradient-primary text-primary-foreground font-semibold text-sm shadow-glow"
          whileTap={{ scale: 0.97 }}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
